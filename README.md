...
- `CLAUDE.md` → `~/.claude/CLAUDE.md` — machine-wide conventions relevant to any Claude session, including casual ones outside `~/dev`.
- `dev_CLAUDE.md` → `~/dev/CLAUDE.md` — conventions relevant to any project under `~/dev` (picked up automatically by Claude Code's directory-walk-up, no per-project duplication needed), but not casual conversations elsewhere.
- `statusline-command.sh` → `~/.claude/statusline-command.sh`. Used to be its own repo (`statusline`) — folded in because it was never really an independent tool: no `install.sh` of its own, no CLI, no lifecycle, just a single script. Design notes (payload schema, known quirks — including a **macOS-only** memory segment, not yet ported to Linux) live in `files/statusline-command.md`, repo-only, not deployed. The `statusLine` wiring in `~/.claude/settings.json` is `claude_config.sh`'s job, not this dotfile's — see below.
- `skills/*/SKILL.md` → `~/.claude/skills/*/SKILL.md` — global Claude Code skills, only loaded on-demand when their situation applies (not on every turn like the CLAUDE.md files above): `claude-config-ownership` (settings.json key ownership + managed-block convention), `shell-scripting` (personal bash conventions), `python-coding` (placeholder, empty until Python conventions are established).
