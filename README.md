...
- `skills/*/SKILL.md` → both `~/.claude/skills/*/SKILL.md` and `~/.agents/skills/*/SKILL.md` — shared global skills for Claude Code and Codex, loaded on demand when their situation applies: `claude-config-ownership` (settings.json key ownership + managed-block convention), `shell-scripting` (personal bash conventions), `python-coding`, and `jupyter-notebooks` (personal .ipynb conventions - built on top of `python-coding` and the bundled `dataviz` skill).
