...
- `codex_config.sh` — manages the Codex status-line selection and transitional command table in `~/.codex/config.toml`, preserving every other Codex setting. The minimized `patches/codex-0.150.1-status-line-command.patch` runs `~/.codex/statusline-command.sh` asynchronously by convention; the provider adapter rotates the three Claude-style lines through Codex's single-line area every four seconds. `CODEX_STATUS_LINE_COMMAND` can override the script path. The table remains temporarily so the currently deployed, older patched binary keeps working until it is replaced.
